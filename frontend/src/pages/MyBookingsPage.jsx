    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {filteredBookings.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <Link
            to="/booking"
            className="mt-4 inline-block px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 transition"
          >
            Book a Court
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {filteredBookings.map((booking) => (
            <div key={booking._id} className="card hover:shadow-lg transition-shadow p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                {/* Booking Details */}
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-3">
                    <h3 className="font-semibold text-lg text-gray-900">{booking.court?.name}</h3>
              ...
              {booking.pricingBreakdown && (
                <details className="mt-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                    ...
                    <span className="text-right">${booking.pricingBreakdown.courtFee?.toFixed(2) || '0.00'}</span>
